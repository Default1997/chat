...
<div class="" align="center">
    <h2>Палуба</h2>
    <p class="lead">Здесь ты можешь пообщаться с командой</p>
</div>
...
<?php   if(in_array(\Yii::$app->user->identity->id, $adminList)){ ?>
            <div id="users_list">
                <table class="table">
            ...
            foreach ($allMessages as $msg): 
                if ($msg['status_message'] == 'displayed') {
                    if (in_array($msg['author_id'], $adminList)){
                        ...
                        if (in_array(\Yii::$app->user->identity->id, $adminList) && !Yii::$app->user->isGuest){ ?>
                    ...
                    }elseif ($msg['status_message'] == 'notDisplayed' && in_array(\Yii::$app->user->identity->id, $adminList)) {
...
<?php 
    if(Yii::$app->user->isGuest) { 
        ...
        <div class="row" id="chat_form">
            <?php $form = ActiveForm::begin();?>
            <?= $form->field($form_model, 'message')->label(false);?>
            <?= Html::submitButton('Отправить', ['class' => 'btn btn-success', 'style' => 'width: 1140px']);?>
            <?php ActiveForm::end() ?>
        </div>
    <?php } ?>
<!-- </div> -->